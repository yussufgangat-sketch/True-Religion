...
export default function MaleProductsPage() {
  ...
  const [filteredProducts, setFilteredProducts] = useState(products.filter(product => product.category === "male"));
  const [groupedProducts, setGroupedProducts] = useState<Record<string, typeof products>>({});

  // Scroll position restoration
  useEffect(() => {
    const scrollPosition = sessionStorage.getItem('male-products-scroll-position');
    if (scrollPosition) {
      window.scrollTo(0, parseInt(scrollPosition));
      sessionStorage.removeItem('male-products-scroll-position');
    }
  }, []);
  ...
  useEffect(() => {
    const handleRouteChange = () => {
      sessionStorage.setItem('male-products-scroll-position', window.scrollY.toString());
    };
  ...
  useEffect(() => {
    // Get categories for men
    const maleCategories = getCategoriesForGender("male");
    setCategories([
      { id: "all", name: "All Products", count: products.filter(p => p.category === "male").length, icon: "🛍️" },
      ...maleCategories
    ]);
    
    // Initialize grouped products with all male products
  ...
  useEffect(() => {
    ...
    const filtered = filterProductsByCategory(products, selectedCategory === "all" ? null : selectedCategory, "male");
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            <p className="text-lg text-gray-600 font-normal max-w-2xl mx-auto leading-relaxed">
              Premium denim for the modern gentleman
            </p>
          </div>
